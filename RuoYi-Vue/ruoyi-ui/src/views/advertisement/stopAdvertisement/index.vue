...
export default {
  ...
  data() {
    return {
      ...
      // 查询参数
      queryParams: {
        beginSurveyTime:null, //采集开始时间
        endSurveyTime:null, //采集结束时间
        pageNum: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    //驳回初审
    handleReject(row){
      this.records.adId = row.id;
      this.$prompt('请输入驳回理由', '驳回初审', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(({ value }) => {
        this.records.auditComments = value;
        this.records.auditStatus = 5
        console.log("驳回理由："+value)
    ...
    getList() {
      this.loading = true
      //审核状态：合法广告页面： 3初审违法    6复审合法  8自动复审合法
      this.queryParams.auditStatuses=[3,6,8]
