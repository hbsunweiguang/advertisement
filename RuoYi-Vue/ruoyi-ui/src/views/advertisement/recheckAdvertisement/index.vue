<template>
  <div class="app-container">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="合法广告" name="second"></el-tab-pane>
      <el-tab-pane label="叫停广告" name="third"></el-tab-pane>
    </el-tabs>
...
<!--      <el-table-column label="备注" align="center" prop="remark" />-->
    ...
    <el-dialog title="查看历史追朔" :visible.sync="showRecords" width="65%" append-to-body >
      <el-table v-loading="loading" :data="recordsList" @selection-change="handleSelectionChange" class="myRecords">
    ...
    <!-- 广告复审 -->
    <el-dialog title="广告复审" :visible.sync="showApprove" width="75%" append-to-body @close="cancelByApprove">
          ...
          <el-descriptions-item label="审核状态" >
              <el-form-item label="" prop="auditStatus">
                <el-select v-model="records.auditStatus" placeholder="请选择审核状态"
                  ...
                  <el-option label="复审合法" value="4" />
                  <el-option label="驳回初审" value="5" />
                  <el-option label="复审违法" value="6" />
                </el-select>
              </el-form-item>
    ...
    <!-- 显示法条列表 -->
    <el-dialog title="广告初步审核" :visible.sync="showClauses2" width="55%" append-to-body>
...
<script>
import { listAdvertisement, getAdvertisement,getIndustry,  getMedium,
  updateAdvertisement,allRegional,getUserList,approveAdvertisement } from "@/api/advertisement/advertisement"
...
export default {
  ...
  created() {
    //审核状态：广告审核状态：1初审合法 2初审存疑 3初审违法
    this.queryParams.auditStatuses=[1,2,3]
    //加载数据
    this.getList()
    //获取省级行政:默认展示河北省
  ...
  methods: {
    ...
    handleClick(tab, event) {
      ...
      if (this.activeName=="second"){
        //审核状态：广告审核状态：4复审合法 7自动复审合法
        this.queryParams.auditStatuses=[4,7]
      }else if (this.activeName=="third"){
        //审核状态：广告审核状态：6复审违法 8自动复审违法
        this.queryParams.auditStatuses=[6,8]
      }
      //获取数据
      this.getList()
    },
    ...
    submitFormByApprove() {
      ...
      this.$refs["formByApprove"].validate(valid => {
        if (valid) {
          let submit = true
          //验证法条
          if (this.records.auditStatus==6 && this.clausesList.length<=0) {
            //let msg = this.records.auditStatus==2?'初审存疑':'初审违法'
            this.$message.error("复审违法时，请选则违法法条！")
            submit = false
          }
          //提交表单
          if (submit){
    ...
    showDetail(row) {
      ...
      if(row.checkStatus==0){
        updateAdvertisement({id:row.id,checkStatus:1,checkTime:new Date()})
          .then(response => {
